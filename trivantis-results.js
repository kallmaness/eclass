...
function ObjBuildResultsHeader(objTest, titleMgr)
{
	...
	for(var index = 0; index < this.m_testResProp.length; index++)
	{
		...
		{
			case "lmsstudentname":
				tempStr += '<p class="'+this.name+'generalsetting"><span class="'+this.name+this.m_testResProp[index]+'generalsetting">'+titleMgr.strAiccSN+'</span></p>';
			break;
			case "testname":
...
function MultRespCorrStatus(objQuestion, titleMgr)
{
	...
	else if(objQuestion.type == titleMgr.DD)	// Drag and Drop
	{
		for(var corIndex = 0; corIndex < objQuestion.arCorrAns.length; corIndex++)
		{
			for(var stuIndex = 0; stuIndex < objQuestion.arCorrAns.length; stuIndex++)
			{
				if(objQuestion.arCorrAns[corIndex].join('-') == stuAns[stuIndex])
					corrCount++;
			}
		}
	}
	...
	if(corrCount == 0)
		return 0;	//All Wrong
	else if (corrCount == objQuestion.arCorrAns.length)
