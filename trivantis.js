...
function ObjLayerUpdateTabIndex(objLayer){
	...
	if( isOffPage(objLayer) || isOffPage(objLayer.theObj) ){
		...
		for(var i = 0 ; i < objDivAnchors.length ; i++){
			objDivAnchors[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < objDivButtons.length ; i++){
			objDivButtons[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < objDivSelects.length ; i++){
			objDivSelects[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < objDivInputs.length ; i++){
			objDivInputs[i].tabIndex = objLayer.theObj.z;
		}
		
		for(var i = 0 ; i < eleAnchors.length ; i++){
			eleAnchors[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < eleButtons.length ; i++){
			eleButtons[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < eleSelects.length ; i++){
			eleSelects[i].tabIndex = objLayer.theObj.z;
		}

		for(var i = 0 ; i < eleInputs.length ; i++){
			eleInputs[i].tabIndex = objLayer.theObj.z;
		}
	}
}
...
function getOrientationFromAngle(){
	...
	var angle = screen.orientation?screen.orientation.angle:win.orientation?win.orientation:0;
	switch(angle) 
	{  
		
		case -90:  
		case 90: 
			if((window.baseOrientation.angle == 0 || window.baseOrientation.angle == 180) && window.baseOrientation.orientation == "landscape")
				screenOri = "portrait";
			else
				screenOri = "landscape";
        break;
...
function buildCSS(id,bFixed,left,top,width,height,visible,zorder,color,other,sizeUnit,bClip) {
  if(typeof(bClip) == 'undefined' || bClip == null )
	bClip = true;
  var str = (left!=null && top!=null)? '#'+id+' {position:'+(bFixed?'fixed;':'absolute;')+'left:'+left+'px;top:'+top+'px;' : ((width!=null && height!=null) ? '#'+id+' {position:relative;' : '#'+id+' {position:fixed;width:100%;height:100%;' )
...
function addSvgShadowFilter(name,width, height, direction, depth, opacity, red, green, blue, blurRadius, type , borderWidth)
{
	...
	if(xOffset <= 0 || yOffset <= 0 || borderWidth)
	{
		...
		if(xOffset <= 0)
		{
			...
			if(borderWidth) xDisplacementPercentage = (((xOffset - blurRadius - (borderWidth*2)) / width) * 100).toFixed(5);   
			svgFilter += 'x = "' + xDisplacementPercentage + '%" '
		}
		else if (borderWidth)
		{
			var xDisplacementPercentage = ((((borderWidth*2)) / width) * 100).toFixed(5);
			svgFilter += 'x = "' + (-1*xDisplacementPercentage) + '%" '
		}
		else svgFilter += 'x = "0" '
		...
		if(yOffset <= 0)
		{
			...
			if (borderWidth) yDisplacementPercentage = (((yOffset - blurRadius  - (borderWidth*2)) / height) * 100).toFixed(5);
		...
		if(!borderWidth)	svgFilter += 'width="'+ 100 * (Math.abs(xOffset) + width + 2*blurRadius) / width +'%" height="'+ 100 * (Math.abs(yOffset) + height + 2*blurRadius) / height +'%">\n';
		else	svgFilter += 'width="'+200+'%" height="'+200+'%">\n';
...
function addReflection(name, src, topLeftX, topLeftY, width, height, angle, offset, fadeRate, visible, 
	...
	if(is.ie8 )
		reflection += '; position:' + (bFixedPosition ? 'fixed;' : 'absolute;') + ' top:' + ie8DivY + 'px; left:' + ie8DivX + 'px; width:' + ie8DivWidth + 'px; height:' + ie8DivHeight + 'px;">\n';
	else
	{
		...
		if(is.awesomium)
			reflection += '; position:' + (bFixedPosition ? 'fixed;' : 'absolute;') + 'top:' + topLeftY + 'px; left:' + topLeftX + 'px; width:' + width + 'px; height:' + height + 'px;">\n';	
		else
			reflection += '; position:' + (bFixedPosition ? 'fixed;' : 'absolute;') + 'top:' + (topLeftY - 1) + 'px; left:' + topLeftX + 'px; width:' + width + 'px; height:' + height + 'px;">\n';	
	}
...
function adjustForFixedPositon(obj, respChange, biOSBarChange){
	...
	if(obj.bFixedPosition && obj.bBottom)
	{
		if(is.iOS && !biOSBarChange)
			...
			var viewHeight = window.top.innerHeight;
		...
		if(isSppInIosFrame())
		{
			...
			obj.y = scrollPosY + (viewHeight - offset);	
		}
		else
		{
			if( pHeight > viewHeight )
			{
				if((obj.origY + obj.h) < pHeight)
					obj.y = (viewHeight - obj.h) - (pHeight - (obj.origY + obj.h));
				else
					obj.y = viewHeight - obj.h;
			}
		}
						...
						for(var i = 0 ; i < fixedDiv.children.length ; i++)
						{
							...
							if(!!parseInt(bBottom)){								
								var pHeight = parseFloat(GetCurrentPageDiv().clientHeight),
									viewHeight = window.top.innerHeight,
									offset = pHeight - origY;
								
								fixedDiv.children[i].style.top = (viewHeight - offset) + "px";
							}
						}
					}
				}, 100);
