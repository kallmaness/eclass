...
TMPr.loadTest = function( fn, tn, pn )
{
  ...
  if( !this.arTests[tIdx].bLoaded )
  {
	...
	if ( httpReq.responseText && httpReq.responseText.search("<?xml") >= 0 ) 
		responseXML = httpReq.responseText;
	else
		responseXML = this.b64DecodeUnicode(httpReq.responseText);
...
TQPr.getMaxScore = function()
{
  ...
  if( !this.bSurvey )
  {
    if( this.bGradeInd )
    {
      if (this.type == jTitleManager.MR || this.type == jTitleManager.HS || this.type == jTitleManager.MC)
